...
enum class CodeFieldState { DEFAULT, ERROR, SUCCESS }
@Composable
fun RotbeYarTextField(
    ...
    OutlinedTextField(
        ...
        enabled = enabled,
        isError = isError,
        label = { RotbeYarText(text = label, fontSize = 12.sp, modifier= Modifier.background(Color.Transparent)) },
        placeholder = { if (placeholder.isNotEmpty())RotbeYarText(text = placeholder, color = PrimaryGray.copy(alpha = ContentAlpha.medium)) },
        supportingText = {
            if (supportingText != null) {
              RotbeYarText(color = MaterialTheme.colorScheme.error
                  , text = supportingText)
            }
        },
        singleLine = true,
    ...
    Row(
        ...
        repeat(length) { index ->
            ...
            OutlinedTextField(
                ...
                colors = OutlinedTextFieldDefaults.colors(
                    focusedBorderColor = if (isSuccess) PrimaryGreen else MaterialTheme.colorScheme.primary,
                    unfocusedBorderColor = if (isSuccess) PrimaryGreen else Color.Gray,
